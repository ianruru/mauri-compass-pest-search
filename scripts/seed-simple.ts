...
for (const pest of pestsData) {
  try {
    await db.insert(pests).values({
      title: pest.Title || "",
      latin: pest.Latin || "",
      alsoKnownAs: pest["Also known as"] || "",
      keywords: pest.Keywords || "",
      pestGroups: pest["Pest groups"] || "",
      pestTypes: pest["Pest types"] || "",
      managementApproaches: pest["Management approaches"] || "",
      alert: pest.Alert === "Yes",
      pinned: pest.Pinned === "Yes",
      visible: pest.Visible === "Yes",
